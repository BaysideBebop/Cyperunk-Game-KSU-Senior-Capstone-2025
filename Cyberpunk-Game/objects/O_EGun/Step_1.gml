	...
	if (point_distance(O_Player.x,O_Player.y,x,y) < 300)
	{
		...
		if (countdown <=0)
		{
			...
			if (!collision_line(x,y,O_Player.x,O_Player.y,O_Wall,false,false))
			{
				...
				if (O_Player.x < x)
				{
					owner.sprite_index = SPR_PlayerIdleLeft;
					owner.looking = -1;
				}
				if (O_Player.x > x)
				{
					owner.sprite_index = SPR_PlayerIdleRight;
					owner.looking = 1;
				}
			}
		}
