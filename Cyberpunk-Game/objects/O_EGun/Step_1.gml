...
if (instance_exists(O_Player))
{
	...
	if (owner.facingright) && (O_Player.x < x) && (owner.stationary) image_xscale = -1;
	if (point_distance(O_Player.x,O_Player.y,owner.x,owner.y) < 400)
	{
		if (!collision_line(O_Player.x,O_Player.y,owner.x-32,owner.y,O_Wall,false,false))
		{
		...
		if (countdown <=0)
		{
			...
			if (!collision_line(owner.x+32,owner.y,O_Player.x,O_Player.y,O_Wall,false,false))
			{
				//Bullet code
